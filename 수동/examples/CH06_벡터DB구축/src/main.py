...
사전 준비:
    1. 샘플 PDF 생성: python data/create_sample_pdfs.py
    2. Ollama 서버 실행: ollama serve
    3. 임베딩 모델 준비: ollama pull nomic-embed-text
    4. 환경 변수 설정: cp .env.example .env
"""
...
from src.store import add_documents, create_collection, get_collection_stats, search
...
def run_pipeline(pdf_paths: list[str], use_vision: bool = False) -> None:
    ...
    # --- Process ---
    collection = create_collection(persist_dir=persist_dir, name=collection_name)
    ...
    print("  CH07 연결: outputs/chroma_db 폴더를 CH07/data/chroma_db 로 복사하십시오.")
    print("=" * 65)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "\n"
            "사전 준비:\n"
            "  python data/create_sample_pdfs.py  # 샘플 PDF 생성\n"
    ...
    # 처리할 PDF 파일 목록
    pdf_files = [
        os.path.join(data_dir, "HR_취업규칙_v1.0.pdf"),
        os.path.join(data_dir, "FIN_매출현황_v1.0.pdf"),
    ]

    # PDF 존재 여부 확인
    missing_pdfs = [p for p in pdf_files if not os.path.exists(p)]
    if missing_pdfs:
        print("다음 PDF 파일이 없습니다:")
        for p in missing_pdfs:
            print(f"  {p}")
        print()
        print("먼저 샘플 PDF를 생성하십시오:")
        print("  python data/create_sample_pdfs.py")
        sys.exit(1)
