...
메타코딩은 CH03에서 RAG의 가능성을 확인하였지만, AI 비서가 답변할 "사내 데이터"가 아직 시스템화되어 있지 않다. 현재 커넥트에서는 직원 정보를 Excel로 관리하고 있고, 휴가 현황은 인사팀이 수동으로 집계하며, 매출 데이터는 부서별로 제각각이다. "AI 비서가 연차를 알려주려면, 먼저 연차 데이터가 DB에 있어야 하지 않을까?"

### 해결 과정에서의 감정/고민

- "AI를 만들기 전에 기본적인 사내 시스템부터 있어야 한다는 것을 깨달았다"
- "React 같은 프론트엔드를 배울 시간이 없다. Python만으로 웹 UI까지 만들 수 없을까?"
- "FastAPI + Jinja2면 Python 파일 몇 개로 CRUD 사이트가 완성된다. 이 정도면 충분하다"
- Admin UI를 통해 직원 30명의 데이터를 직접 입력하고, 브라우저에서 확인하는 순간의 성취감

### before/after 수치 (예상)

| 지표 | Before | After |
|------|--------|-------|
| 직원 정보 관리 | Excel 파일 1개 | PostgreSQL + 웹 Admin UI |
| 휴가 현황 조회 | 인사팀 수동 집계 (30분) | Admin UI에서 즉시 확인 |
| 매출 데이터 접근 | 부서별 개별 파일 | 통합 DB + API |
| 데이터 활용 가능성 | 파일 공유만 가능 | API 기반 자동 조회 (CH08 연계) |
