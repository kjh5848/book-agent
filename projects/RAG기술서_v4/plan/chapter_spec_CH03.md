...
메타코딩은 환경을 구축한 뒤 바로 DeepSeek R1에 사내 질문을 던져 본다. "김철수 사원의 남은 연차가 며칠입니까?" 그러자 LLM은 자신 있는 말투로 "김철수 사원의 잔여 연차는 12일입니다"라고 답한다. 하지만 실제로는 7일이다. 그럴듯하지만 완전히 틀린 답변, 즉 '환각'을 처음 경험한다.

### 해결 과정에서의 감정/고민

- "LLM이 이렇게 자신 있게 거짓말을 하다니... 이대로는 실무에 쓸 수 없다"
- "문서를 통째로 프롬프트에 넣으면 되지 않을까?" -> 3개 문서만 넣었는데 토큰 초과 오류 발생
- "문서를 잘게 쪼개서 필요한 부분만 검색해서 넣으면?" -> RAG의 핵심 아이디어를 직접 체감
- "RAG로 다시 질문하니 '잔여 연차 7일'이라고 정확하게 답변한다!"

### before/after 수치 (예상)

| 지표 | Before (LLM 단독) | After (RAG 적용) |
|------|-------------------|-----------------|
| 사내 정보 질의 정확도 | 0% (환각) | 85%+ (출처 기반) |
| 답변에 출처 포함 | 없음 | 문서명 + 페이지 표시 |
| 프롬프트 토큰 사용량 | 전체 문서 첨부 시 초과 | 관련 청크만 500~1000 토큰 |
