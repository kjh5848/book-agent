# plan.md -- 사내 문서 기반 AI 업무 비서 (RAG + MCP) v4

> 생성일: 2026-02-27
> 상태: 1차 초안
> 기반: v3 plan.md (기술 내용 동일, writing_concept 변경)
...
| story_persona | `메타코딩` -- 1인 개발자, 중소기업(30인 규모) AI 도입 담당 |
| 설명 | 메타코딩이 사내 문서 관리 문제를 해결해 나가는 서사 구조. 각 챕터가 메타코딩이 직면한 구체적 문제 상황으로 시작하여 기술을 도입하고 해결하는 과정을 따라간다. |
| 프로젝트명 | "커넥트HR AI 비서" |
| 스토리 배경 | 중소기업 "커넥트"에서 사내 문서가 3,000건 이상 쌓여 직원들이 필요한 정보를 찾지 못하는 문제가 반복된다. 메타코딩은 이 문제를 AI로 해결하기로 결심한다. |

#### 스토리텔링 구조 규칙

각 챕터는 다음 4단계 구조를 따른다.

| 단계 | 내용 | 비중 |
|------|------|------|
| 1. 문제 상황 | 메타코딩이 직면한 구체적 문제 (수치, 시간, 반복 작업 포함) | 10% |
| 2. 기술 소개 | 문제를 해결할 기술의 개념과 원리 | 20% |
| 3. 해결 과정 | 기술을 적용하는 실습 (코드 + 설명) | 60% |
| 4. 결과 (before/after) | 문제 해결 수치 비교 + 다음 챕터 예고 | 10% |

#### 스토리텔링 톤 규칙

- 하십시오체 유지 (문어체 + 서사 흐름)
- 메타코딩의 서사는 챕터 도입부와 결미부에 집중, 기술 설명 중에는 3줄 이내
- 과장된 감정 표현 금지 ("기적처럼", "드디어 해방" 등 사용 불가)
- before/after 수치는 반드시 구체적 숫자로 제시
...
| 채팅 스타일 | Gemini 스타일 (사용자=파랑 우측, AI=회색 좌측) |
...
```mermaid
flowchart LR
    P1["CH01-03: 문제 인식"] -- "왜 AI가 필요한가" --> P2["CH04-05: 기반 구축"]
    P2 -- "문서를 지식으로" --> P3["CH06-07: RAG 구현"]
    P3 -- "DB까지 통합" --> P4["CH08-09: 통합 에이전트"]
    P4 -- "품질 개선" --> P5["CH10: 완성"]
```

*그림 0-3: 메타코딩의 문제 해결 여정 -- 챕터마다 하나의 문제를 해결한다*
...
| `POSTGRES_DB` | `connect_hr` | CH04~ | 데이터베이스 이름 |
| `POSTGRES_USER` | `connect_hr` | CH04~ | DB 사용자 |
| `POSTGRES_PASSWORD` | `connect_hr_pass` | CH04~ | DB 비밀번호 |
...
| 항목 | v3 | v4 |
|------|-----|-----|
| writing_concept | `project-buildup` | `storytelling` |
| story_persona | (없음) | `메타코딩` (1인 개발자, 중소기업 AI 도입 담당) |
| 챕터 도입부 | 기술 중심 도입 | 메타코딩의 문제 상황으로 시작 |
| 챕터 결미부 | 기능 누적 설명 | before/after 수치 비교 |
| 기술 내용 | 동일 | 동일 |
| 아키텍처 | 동일 | 동일 |
| 환경 명세 | 동일 | 동일 |
| 분량 | 95p | 95p (동일) |
