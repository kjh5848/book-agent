...
def check_ollama() -> bool:
    ...
    target_model = os.getenv("OLLAMA_MODEL", "deepseek-r1:8b")
    ...
    try:
        ...
        if is_ok:
            ...
            has_target = any(target_model in name for name in model_names)
        else:
            model_names = []
            has_target = False
    ...
    if is_ok:
        _print_pass("Ollama", f"{base_url}  모델: [{target_model}]")
    else:
        _print_fail(
            "Ollama",
...
def run_all_checks() -> None:
    ...
    if failed == 0:
        ...
        print("  위의 ❌ 항목 해결 방법을 참고하여 환경을 수정한 뒤")
