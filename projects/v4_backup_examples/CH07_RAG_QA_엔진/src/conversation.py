...
ConversationBufferWindowMemory를 활용하여 최근 N턴의 대화만 유지한다.
"""
...
class ConversationManager:
    ...
    최근 N턴의 대화만 유지하고(ConversationBufferWindowMemory),
    ...
    Attributes:
        ...
        _sessions: 세션 ID → (Memory, 마지막 접근 시각) 딕셔너리
    """
    ...
    def __init__(
        ...
        # 세션 저장소: {session_id: (ConversationBufferWindowMemory, last_access_time)}
        self._sessions: dict[str, tuple[ConversationBufferWindowMemory, float]] = {}
    ...
    def _get_or_create_memory(
        ...
        Returns:
            해당 세션의 ConversationBufferWindowMemory 인스턴스
        """
        # === INPUT ===
        now = time.time()
        ...
        if session_id not in self._sessions:
            ...
            memory = ConversationBufferWindowMemory(
                k=self.window_size,
                return_messages=False,  # 문자열 형식으로 히스토리 반환
                human_prefix="사용자",
                ai_prefix="AI 비서",
            )
