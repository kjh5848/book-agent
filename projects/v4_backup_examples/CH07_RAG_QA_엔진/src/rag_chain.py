...
ChromaDB가 없으면 data/docs/ 원본 문서를 파싱하여 ChromaDB를 자동 구축한다.

사용 방법:
    chain, retriever = get_rag_chain()
    answer = chain.invoke({"question": "연차 규정은?", "history": "없음"})
"""
...
def build_rag_chain() -> tuple[Any, Any]:
    ...
    retriever = _build_retriever()    # ② Retriever 생성 (ChromaDB 또는 자동 구축)
