...
ChromaDB 기반 검색을 사용하려면 CH06 예제를 먼저 완료하십시오.
...
python src/main.py 7    # Vision + OCR 추출
python src/main.py 8    # 평가 프레임워크 데모
...
├── src/
│   ├── __init__.py
│   ├── main.py                # 통합 실행 진입점 (메뉴 선택)
│   └── eval_framework.py      # 평가 프레임워크 (Precision@k, RAGAS)
├── tuning/
│   ├── __init__.py
│   ├── chunk_experiment.py    # Fixed-size vs Semantic 청킹 비교
│   ├── retriever_experiment.py  # k값, threshold, metadata filter
│   ├── reranker.py            # Cross-Encoder ReRanker
│   ├── hybrid_search.py       # BM25 + Vector EnsembleRetriever
│   ├── advanced_retriever.py  # Parent/SelfQuery/ContextualCompression
│   ├── query_rewrite.py       # HyDE, Multi-Query, 약어/동의어
│   └── vision_extractor.py    # LLaVA + EasyOCR 하이브리드
├── data/
│   └── test_questions.json    # 테스트 질문 30개 (정형10+비정형10+복합10)
└── outputs/
    └── .gitkeep               # 실험 결과 저장 위치
```
