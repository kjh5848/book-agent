...
PostgreSQL과 ChromaDB 없이도 **인메모리 샘플 데이터**로 즉시 실행 가능합니다.

---

## 빠른 시작 (인메모리 모드)

PostgreSQL, Ollama 없이 바로 테스트할 수 있습니다.
...
### 2단계: PostgreSQL 시작 (선택)

```bash
docker-compose up -d
```

PostgreSQL 없이도 인메모리 샘플 데이터로 동작합니다.
...
│   ├── mcp_tools.py        # MCP 도구 4개 + 인메모리 폴백
│   └── agent.py            # ReAct 통합 에이전트
├── app/
│   ├── __init__.py
│   ├── database.py         # PostgreSQL 연결 (안전한 폴백 포함)
...
**다음 코드는 DB 연결 실패 시 인메모리 샘플 데이터로 자동 대체합니다.**
...
return next((r for r in sample if emp_no in (r["emp_no"], r["name"])), {"error": "미발견"})  # ④
```

> ① PostgreSQL 조회 시도 (connect_timeout=3으로 빠른 실패)
> ② DB 결과가 있으면 즉시 반환
> ③ DB 없으면 하드코딩된 샘플 데이터 로드
> ④ 이름 또는 번호로 매칭하여 반환
...
| 조건 | 동작 |
|------|------|
| PostgreSQL 미연결 | 하드코딩된 샘플 10명 직원 데이터 사용 |
| ChromaDB 미연결 | 키워드 점수 기반 8개 HR 문서 검색 |
| LLM 미연결 | Step 1, 2만으로 라우팅 결정 |
| AgentExecutor 실패 | LLM 직접 호출 폴백 |
...
- Python 3.11 이상
- (선택) Docker Desktop: PostgreSQL 컨테이너 실행
- (선택) Ollama: 로컬 LLM 실행
- (선택) OpenAI API 키: 클라우드 LLM 사용
