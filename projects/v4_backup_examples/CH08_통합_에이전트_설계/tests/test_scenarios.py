...
인메모리 모드(PostgreSQL, LLM, ChromaDB 없이)에서 18개 테스트를 실행한다.
실행 방법:
...
# ---------------------------------------------------------------------------
# 2. 정형 시나리오 테스트 (DB 조회 → 인메모리 폴백)
# ---------------------------------------------------------------------------

class TestStructuredScenarios(unittest.TestCase):
    """정형 데이터 MCP 도구 인메모리 폴백 테스트."""
...
# ---------------------------------------------------------------------------
# 3. 비정형 시나리오 테스트 (문서 검색 → 인메모리 폴백)
# ---------------------------------------------------------------------------

class TestUnstructuredScenarios(unittest.TestCase):
    """비정형 문서 검색 MCP 도구 인메모리 폴백 테스트."""
    ...
    def test_scenario_07_welfare_info(self) -> None:
        """시나리오 07: 복지 혜택 안내 문서를 검색한다."""
        result = search_documents.invoke({"query": "복지 혜택 건강검진", "k": 3})
        self.assertIsInstance(result, dict)
        results = result.get("results", [])
        self.assertGreater(len(results), 0)

    def test_scenario_08_business_trip_policy(self) -> None:
        """시나리오 08: 출장 규정 문서를 검색한다."""
        result = search_documents.invoke({"query": "출장 규정 숙박 식비", "k": 3})
        self.assertIsInstance(result, dict)
        results = result.get("results", [])
        self.assertGreater(len(results), 0)
        contents = " ".join(r["content"] for r in results)
        self.assertTrue("출장" in contents)
...
class TestHybridScenarios(unittest.TestCase):
    ...
    def test_scenario_09_sales_dept_welfare(self) -> None:
        """시나리오 09: 매출 상위 부서 조회 + 복지 정책 검색을 병렬 실행한다."""
        # 정형: 전체 매출 집계
        ...
        self.assertIn("welfare_docs", combined)
...
class TestMcpToolsUnit(unittest.TestCase):
    ...
    def test_inmemory_search_keyword_match(self) -> None:
        """인메모리 검색이 키워드 기반으로 정렬된 결과를 반환한다."""
        results = _inmemory_search("온보딩 입사", k=3)
