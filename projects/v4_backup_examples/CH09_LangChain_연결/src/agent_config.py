...
# --- 시스템 프롬프트 ---
SYSTEM_PROMPT = """당신은 커넥트HR AI 비서입니다.
...
def _classify_route(query: str) -> str:
    """질문 유형을 판단하여 라우팅 경로를 결정합니다.

    키워드 기반 규칙으로 질문 유형을 분류합니다.
    LLM 호출 없이 동작하므로 지연 없이 빠르게 분류합니다.
    ...
    if db_score > 0 and rag_score == 0:
        ...
        # 복합 질문이거나 명확하지 않으면 Agent에 위임
        route = "agent"

    # --- OUTPUT ---
    logger.info(
        "[Router] 쿼리 분류 완료: route=%s (DB점수=%d, RAG점수=%d)", route, db_score, rag_score
    )
    return route
