...
def load_embedding_model(model_name: str = DEFAULT_EMBEDDING_MODEL) -> SentenceTransformer:
    ...
    print("  최초 실행 시 약 400MB 다운로드가 발생합니다.")
...
def embed_chunks(
    ...
    # 배치 단위 임베딩 계산
    print(f"  {len(documents)}개 청크 임베딩 계산 중... (배치 크기: {BATCH_SIZE})")
    all_embeddings = []
    for batch_start in range(0, len(documents), BATCH_SIZE):
        ...
        print(f"    배치 완료: {done}/{len(documents)}개", end="\r", flush=True)

    print(f"  임베딩 계산 완료: {len(all_embeddings)}개 벡터 생성")
