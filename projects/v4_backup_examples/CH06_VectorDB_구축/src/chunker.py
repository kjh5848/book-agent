...
# =====================================================================
# === INPUT ===
# extract_result: extractor.py 또는 vision_extractor.py의 반환 딕셔너리
...
def build_image_caption_chunk(
    ...
    Vision LLM이 추출한 이미지/표/차트 설명을 일반 텍스트 청크와
...
def chunk_extract_result(
    ...
    """extractor.py 또는 vision_extractor.py의 추출 결과를 청크 리스트로 변환합니다.

    각 페이지의 텍스트를 Fixed-size 청킹으로 분할하고,
    Vision LLM이 추출한 이미지 캡션은 별도 이미지 캡션 청크로 변환합니다.
    모든 청크에 메타데이터를 부착하여 출처 추적이 가능한 형식으로 반환합니다.

    Args:
        extract_result: extractor.extract_text() 또는
                        vision_extractor.extract_with_vision() 반환값
