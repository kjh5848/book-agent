...
├── .env.example
├── data/
│   └── docs/                         ← 사내 문서 원본 (legacy 파일 그대로 사용)
│       ├── hr/
│       │   ├── HR_취업규칙_v1.0.pdf
│       │   └── HR_정보보안서약서.pdf
│       ├── ops/
...
레포에 포함된 실제 사내 문서 샘플 파일들입니다.
별도 생성 스크립트 없이 그대로 사용합니다.

| 파일 | 용도 |
|------|------|
| `hr/HR_취업규칙_v1.0.pdf` | 규칙 기반 파싱 실습 (텍스트 레이어 있음) |
| `hr/HR_정보보안서약서.pdf` | 추가 HR 문서 (부서 필터 검색 실습) |
| `ops/OPS_신규서비스_런칭전략.pdf` | Vision LLM 실습 (복합 레이아웃 체험) |
| `finance/*.xlsx` | CH05 네이밍 규칙 예시 (CH06 파이프라인 외) |
| `security/SEC_보안규정_v1.0.docx` | CH05 네이밍 규칙 예시 (CH06 파이프라인 외) |

`main.py`는 `data/docs/` 하위의 **모든 PDF 파일을 자동 탐색**합니다.
...
**기대 출력 (규칙 기반)**:
```
[1/5] PDF 확인... ✓ HR_취업규칙_v1.0.pdf, HR_정보보안서약서.pdf, OPS_신규서비스_런칭전략.pdf
[2/5] 규칙 기반 파싱 중... ✓ (N페이지 추출)
[3/5] 청킹 중... ✓ (총 N개 청크)
[4/5] 임베딩 중... ✓ (N개 벡터, 차원: 768)
[5/5] ChromaDB 저장 중... ✓ (N개 문서 → outputs/chroma_db/)

=== 검색 테스트 ===
Q: "연차 신청은 어떻게 하나요?"
결과 1 (거리 0.12): "연차는 사용 전월 말일까지..." [HR_취업규칙_v1.0.pdf]
```
...
```
# PDF 처리
pymupdf>=1.24.0          # Vision 모드 이미지 캡처 (fitz)
