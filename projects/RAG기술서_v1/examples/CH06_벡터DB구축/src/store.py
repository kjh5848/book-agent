...
def get_client(persist_dir: str = _DEFAULT_PERSIST_DIR) -> chromadb.PersistentClient:
    ...
    지정된 디렉토리에 데이터를 영속 저장하는 클라이언트를 생성합니다.
    ...
    Returns:
        초기화된 ChromaDB PersistentClient 인스턴스.

    Raises:
        RuntimeError: ChromaDB 클라이언트 초기화에 실패했을 때.
    """
    # --- Input ---
...
def create_collection(
    ...
    Args:
        client: get_client()로 생성한 ChromaDB PersistentClient 인스턴스.
    ...
    Raises:
        ...
        RuntimeError: 컬렉션 생성 또는 접근에 실패했을 때.
    """
    # --- Input ---
...
def add_documents(
    ...
    # --- Output ---
    print(f"  {saved_count}개 청크 저장 완료. 컬렉션 총 문서 수: {collection.count()}")
    return saved_count
...
def get_collection_stats(collection: chromadb.Collection) -> dict:
    ...
    Returns:
        통계 딕셔너리:
            {
                "total": int,            # 총 문서(청크) 수
                "by_department": dict    # 부서별 문서 수 {department: count}
            }
    ...
    if total > 0:
        try:
            ...
            by_department = dict(Counter(departments))
        except Exception as e:
            raise RuntimeError(
