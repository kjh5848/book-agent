...
class Employee(Base):
    ...
    # 연관 관계 — SQLAlchemy 2.x에서 비매핑 어노테이션 허용 (__allow_unmapped__ = True)
...
class Leave(Base):
    ...
    # 연관 관계 — SQLAlchemy 2.x에서 비매핑 어노테이션 허용 (__allow_unmapped__ = True)
    employee: ClassVar["Employee"] = relationship("Employee", back_populates="leaves")
...
class Sale(Base):
    ...
    # 연관 관계 — SQLAlchemy 2.x에서 비매핑 어노테이션 허용 (__allow_unmapped__ = True)
    employee: ClassVar["Employee"] = relationship("Employee", back_populates="sales")
