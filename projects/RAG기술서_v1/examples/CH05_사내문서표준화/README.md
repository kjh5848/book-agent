...
│   └── sample_clean/
│       ├── hr_policy_clean.txt        ← 전처리 후 정제 문서 (텍스트 형식)
│       └── HR_취업규칙_v1.0.md        ← 네이밍 표준 + Markdown 변환 최종본
└── guides/
    ├── collection_strategy.md         ← 문서 유형별 수집 전략 + 코드 예시
    ├── preprocessing_rules.md         ← 전처리 규칙 표 + Python 전처리 함수
    └── naming_convention.md           ← 네이밍 표준 규칙 + 메타데이터 파싱 함수
```
...
`hr_policy_raw.txt`에 `guides/preprocessing_rules.md`의 전처리 파이프라인을 적용한 결과물입니다.
...
`hr_policy_clean.txt`에 네이밍 규칙(`{부서}_{문서명}_{버전}.확장자`)을 적용하고 Markdown 형식으로 변환한 최종본입니다.

- 파일명이 `HR_취업규칙_v1.0.md` 형식으로 표준화되었습니다.
- `#`, `##` 헤더와 `|` 표 등 Markdown 마크업이 적용되었습니다.
- 6장에서 ChromaDB에 직접 입력하는 형식의 문서입니다.
...
사내 문서 파일 네이밍 표준 규칙과 파일명에서 메타데이터를 자동 파싱하는 Python 함수를 제공합니다.

- 포맷: `{부서}_{문서명}_{버전}.확장자`
- 올바른 사례 vs 잘못된 사례 비교표가 포함됩니다.
- `parse_filename_metadata()`: 파일명에서 부서, 문서명, 버전을 자동 추출합니다.
- 이 함수는 6장 `extractor.py`에 그대로 이식됩니다.
...
| `naming_convention.md`의 `parse_filename_metadata()` | `extractor.py` 메타데이터 자동 추출 |
