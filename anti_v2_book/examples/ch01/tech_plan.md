...
├── requirements.txt      # 필요한 패키지 (langchain-community 등)
...
1. 사용자가 사전에 Ollama와 `deepseek-r1` 모델을 설치했다고 가정합니다 (또는 시스템 구동 전 확인).
2. 가상환경 활성화 후 `pip install -r requirements.txt` 실행.
3. `python 01_basic_llm.py` 실행 시 로컬 모델이 응답을 반환하는지 로그 확인. (사내 규정 등 모델이 모르는 정보를 물었을 때 엉뚱한 대답이 나오는 것을 확인)
4. 이상 없이 실행되면 해당 터미널 결과를 캡처하여 `assets/CH01_result_hallucination.png` 로 저장.
